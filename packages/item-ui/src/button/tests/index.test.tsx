...
import { Button } from 'unisc-item-ui'
...
describe('button', () => {
  it('type', () => {
    ...
    expect(button.element.classList.contains('item-button--primary')).toBe(true)
    // 测试完成后我们记得要销毁一下元素
    wrapper.unmount()
    ...
    expect(button1.element.classList.contains('item-button--dashed')).toBe(true)
    wrapper1.unmount()
  })
  ...
  it('size', () => {
    ...
    const button = wrapper.find('button')
    expect(button.element.classList.contains('item-button-size--small')).toBe(true)
    wrapper.unmount()
  })
